...
int main(int argc, char* argv[])
{
  ...
  seed_seq seed_sequence(args.seeds.begin(), args.seeds.end());
  ...
  auto our_matrix = cs340::create_random_matrix(args.tasks, args.machines, 30, engine);
  ...
  ) 
  {
    ...
    /* TODO: WRITE CODE HERE */
	auto cpu_time_one = chrono::high_resolution_clock::now();
	auto result = cs340::run_simulation(our_matrix, params, engine);
	
	auto cpu_time_two = chrono::high_resolution_clock::now();
	auto cpu_time_total = cpu_time_two - cpu_time_one;
	chrono::duration<double> elapsed_seconds{cpu_time_total};
    ...
    /* TODO: WRITE CODE HERE */
std::cout << params.pool_size << '\t' << std::setw(15) << left << result.score(our_matrix) << '\t' << elapsed_seconds.count() << std::endl;
  }
}

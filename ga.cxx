...
namespace cs340 
{
  namespace 
  {
    ...
    struct schedule_compare 
    {
      ...
      bool operator () (schedule const& s1, schedule const& s2) const {
		return (s1.score(matrix_) > s2.score(matrix_));
      }	
      ...
      size_t const pool_size, random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE */
	  std::uniform_int_distribution<std::size_t> distribution{0, matrix.machines()};
	  ...
	  std::generate_n(std::back_inserter(schedules), pool_size, [&gen, &distribution, &matrix] () {
		  schedule c{matrix.tasks()};
		  for (std::size_t  i = 0; i < matrix.tasks(); ++i) {
			  c.set_task_assignment(i, distribution(gen));
		  }
    ...
    auto cross_over(schedule c1, schedule const& c2, random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE */
	  std::uniform_int_distribution<std::size_t> distribution{0, c1.tasks()};
      ...
      /* TODO: WRITE CODE HERE */
	  auto copoint = distribution(gen);
	  
	  for (std::size_t i = copoint; i < c1.tasks(); ++i) {
        ...
        random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE */
	  std::uniform_int_distribution<std::size_t> task_distribution{0, c.tasks()},
		machine_distribution{0, matrix.machines()};
		
	  auto pTask = task_distribution(gen), pMachine = machine_distribution(gen);
	  
	  c.set_task_assignment(pMachine, pTask);
      ...
      vector<schedule>& gene_pool, random_generator& gen)
    {
      ...
      /* TODO: WRITE CODE HERE */
	  auto x_pairs_count = x_pairs_dist(gen);
	  ...
	  if (x_pairs_count > 0 && x_pairs_count < gene_pool.size()) {
        ...
        /* TODO: WRITE CODE HERE */
		auto newIter = gene_pool.begin();
		newIter += x_pairs_count;
		
		gene_pool.erase(newIter, gene_pool.end());
		...
		std::transform(gene_pool.begin(), gene_pool.end(), std::back_inserter(totals), [&matrix] (auto const& c) {
			return c.score(matrix);
		});
        ...
        /* TODO: WRITE CODE HERE */
		uniform_real_distribution<double> real_dist{0, totals.back()};
		...
		for (std::size_t i = 0; i < x_pairs_count; ++i) {
          ...
          /* TODO: WRITE CODE HERE */
		  auto rand1 = real_dist(gen), rand2 = real_dist(gen);
          ...
          /* TODO: WRITE CODE HERE */
		  auto first_value = std::lower_bound(totals.begin(), totals.end(), rand1);
          ...
          /* TODO: WRITE CODE HERE */
		  auto dist1 = std::distance(totals.begin(), first_value);

          // 2e iv. Perform the two above steps again. This time using the
          // second random number you drew.

          /* TODO: WRITE CODE HERE */
		  auto second_value = std::lower_bound(totals.begin(), totals.end(), rand2);
		  auto dist2 = std::distance(totals.begin(), second_value);
          ...
          /* TODO: WRITE CODE HERE */
		  auto iter1 = gene_pool.begin();
		  iter1 += dist1;
		  
		  auto iter2 = gene_pool.begin();
		  iter2 += dist2;
          ...
          /* TODO: WRITE CODE HERE */
		  auto new_schedule = cross_over(*iter1, *iter2, gen);
		  ...
		  auto first_schedule = std::lower_bound(gene_pool.begin(), gene_pool.end(), new_schedule, schedule_compare(matrix));
		  gene_pool.insert(first_schedule, new_schedule);
      ...
      /* TODO: WRITE CODE HERE */
	  auto num_mutations = mut_dist(gen);
      ...
      for (size_t j{}; j < num_mutations; ++j) 
      {
        ...
        /* TODO: WRITE CODE HERE */
		auto sampled_sol = m_sel_dist(gen);
		auto gene_iter = gene_pool.begin();
		gene_iter += sampled_sol;
		...
		// NOT SURE WHAT TO DO HERE
		auto pos = std::lower_bound(gene_pool.begin(), gene_pool.end(), *gene_iter, schedule_compare(matrix));
		...
		// ASK PROFESSOR HERE TOO
		if (pos > gene_iter) {
			std::rotate(gene_iter, pos, pos);
		}
		else {
			std::rotate(pos, pos, gene_iter);
		}
      }
    }
      ...
      random_generator& gen)
  {
    ...
    /* TODO: WRITE CODE HERE */
		...
		return run_simulation_n_times(matrix, gene_pool, args.generations, gen); // NOT PASSING DEFAULT LAST PARAMETER
	}
    ...
    /* TODO: WRITE CODE HERE */
	uniform_int_distribution<std::size_t> seed_gen{0, 100};
    ...
    for (size_t i{}; i < args.threads; ++i) 
    {
      ...
      /* TODO: WRITE CODE HERE */
	  std::vector<std::size_t> seeds{seed_gen(gen), seed_gen(gen), seed_gen(gen),
		seed_gen(gen), seed_gen(gen), seed_gen(gen)};
      ...
      future_winners.push_back(
        async(
          ...
          [&matrix, &args, seeds = move(seeds), i] () -> schedule
          {
            ...
            /* TODO: WRITE CODE HERE */
			cs340::random_generator engine(seed_sequence);
            ...
            /* TODO: WRITE CODE HERE */
			auto gene_pool = populate_gene_pool(matrix, pool_size, engine);
            ...
            /* TODO: WRITE CODE HERE */
			return run_simulation_n_times(matrix, gene_pool, args.generations, engine); // AGAIN NOT PASSING LAST PARAMETER
          }
        )
      );
    }
	...
	for (auto& value : future_winners) {
		auto&& temp = value.get();
		if (schedule_compare{matrix}(temp, best)) {
			best = std::move(temp);
		}
	}
